...
usage(){
    echo "Usage: $0 [-d build_dir] [-b build_type] [-c compiler] [-t] [-r] [-h]"
...
while getopts 'd:b:c:htr' OPTION; do
    case ${OPTION} in
...
echo "Selected options: build_dir=${BUILD_DIR}, build_type=${BUILD_TYPE}, compiler=${COMPILER}, build and run tests=${RUN_TESTS}, rebuild libmagic=${REBUILD_MAGIC}"

cmake -DCMAKE_BUILD_TYPE:STRING=${BUILD_TYPE} -DBUILD_MAGICXX_TESTS=${RUN_TESTS} -DREBUILD_MAGIC=${REBUILD_MAGIC} -DCMAKE_CXX_COMPILER:FILEPATH=${COMPILER} -G Ninja -S . -B ${BUILD_DIR} || {
    exit 2
}
